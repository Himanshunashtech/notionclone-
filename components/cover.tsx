...
export const Cover = ({ url, preview }: CoverImageProps) => {
  ...
  return (
    <div
      className={cn(
        "group relative z-10 w-full overflow-hidden",
    ...
    >
      ...
      {url && !preview && canHover && (
        <div className="absolute right-5 bottom-5 flex items-center gap-x-2 opacity-0 group-hover:opacity-100">
          <Button
            onClick={() => coverImage.onReplace(url, params.documentId as string)}
        ...
        <div className="absolute right-2 bottom-2 flex items-center">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" alignOffset={7} sideOffset={5}>
              <DropdownMenuItem onClick={() => coverImage.onReplace(url, params.documentId as string)}>
                <ImageIcon className="mr-2 h-4 w-4" />
                Change cover
              </DropdownMenuItem>
