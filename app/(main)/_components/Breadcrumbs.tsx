      ...
      {ancestors.map((doc, index) => {
        ...
        return (
          <div key={doc._id} className="flex items-center gap-x-1.5">
            ...
            >
              {doc.icon ? (
                <span className="flex items-center justify-center shrink-0 h-4.5 w-4.5">
                  <DocumentIcon icon={doc.icon} className="h-4.5 w-4.5 text-base" />
                </span>
              ) : isRoot ? (
                <span className="flex h-4.5 w-4.5 items-center justify-center rounded bg-neutral-200 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 shrink-0">
